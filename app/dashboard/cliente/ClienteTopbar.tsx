...
// Topbar del cliente: solo nombre + email + avatar.
// NO expone la botonera "FIRMA" del valuador porque el cliente no firma.
export default function ClienteTopbar({ paginaActiva }: { paginaActiva?: string }) {
    ...
    <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0">
      <div className="flex items-center gap-8">
        ...
        <nav className="flex gap-6 text-sm h-full items-center">
          <span className="font-bold border-b-2 border-slate-900 h-full flex items-center pt-[2px]">
            {paginaActiva ?? 'Portal cliente'}
          </span>
        </nav>
      </div>
      <div className="flex items-center gap-3 border-l border-slate-200 pl-6">
        ...
        <div className="w-8 h-8 rounded-full overflow-hidden">
          <div className="w-full h-full bg-sky-700 text-white flex items-center justify-center text-xs font-bold">
